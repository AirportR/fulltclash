...
- 测试延迟的超时界限从10000ms改为5000ms，超过5000则会被判定为超时

- 增加流媒体检测重连（重测）机制，部分节点由于网络各方面原因，可能第一次会检测失败，现在，如果连接错误，会根据重新测的次数(reconnection)进行重新检测，reconnection次数默认为一次。

- 进度条改进，现如今可以在进度百分比后面实时查看已测节点数量，
