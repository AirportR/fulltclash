...
- 节点链路拓扑支持多个clash核心进程并行测试。体现在测试速度大幅加快。与流媒体测试同理，核心数越大，速度越快，占用内存越多。
- 新增geoip-api配置。目前可选择 ip.sb 与 ip-api.com 两个api。具体查看config.yaml.example
- 优化测试进度反馈
- 修复DNS端口占用问题。
- 修复代理问题。现如今获取订阅与bot通讯的代理已分离。注意！现如今bot通讯代理设置已更改为:
```
bot:
 proxy: 127.0.0.1:7890
```
- 修复节点拓扑测试图无法正常显示测试耗时。
- 已将节点落地分析默认api改为 ip-api.com。
